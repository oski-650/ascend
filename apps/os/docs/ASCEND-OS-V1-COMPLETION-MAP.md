...
| Dependency D1 Prospect source-correct mutations — **TECHNICALLY COMPLETE (2026-09-20), pending owner acceptance of D1b.2**: D1a `4276f89`, D1b.1 `eb0f8b6`, D1b.2 (this checkpoint) | Promotion/deletion honor selected owner; truthful partial outcomes and retry contract. D1b replaced Postgres-mode deletion with ARCHIVAL (`009_prospect_archival.sql`) and closed D1a's concurrency debt with a prospect-scoped transaction advisory lock, proven on a real PostgreSQL 17.6 server: without the lock, 2 concurrent promotions produced 2 clients and 6 produced 6; with it, one client and one `prospect.promoted`. D1b.2 applied 009 to production (622 ms, one transaction), verified 51/51, took the new current recovery point and re-proved it through full R1b and two-leg R1c. Zero business rows changed; zero prospects archived. | R1; explicit source configuration | Vault/Postgres cases, retained notes, no silent closed-won disagreement; no destructive migration. **Deployment invariant SATISFIED: D1a/D1b.1 code is DEPLOYABLE AGAINST SCHEMA 009 — but NOT DEPLOYED and NOT AUTHORIZED.** The running service is still the 2026-09-18 build and works unchanged against 009. Carried: sales archival not yet witnessed against a production sales principal (none provisioned); R1c AC4 compares the ACTIVE-set reader to all restored rows and will need re-scoping once the first prospect is archived. |
