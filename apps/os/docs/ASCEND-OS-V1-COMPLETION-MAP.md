...
| DB substrate — KEEP | `core/db/client.ts`, migrations 001–009, scoped transactions, prospect/note event writes. | Production ledger **inspected and verified 2026-09-20 (D1b.2)**: 001–009, zero checksum drift, archival grants/RLS shape verified 51/51. Other settings not re-audited. | Verify deployment separately; keep RLS, migration checksums, transactional writes. |
...
| ↳ Backup/recovery status after R1a (2026-09-19) | `scripts/backup-production.sh`, `core/recovery/`, `scripts/recovery-verify.sh`. `core/db/backup.ts` RETIRED. | One canonical mechanism; encrypted artifact; restore proven on 001–008 with every table populated. No current production recovery point until R1b. | R1b, then R1c. Vault → R2. |
| ↳ **Dependency R1 — PostgreSQL Recovery: COMPLETE (2026-09-19)** | R1a `e95e3bd`, R1b `dd46b95`, R1c `680d94e`; `docs/DEPENDENCY-R1{A,B,C}-CHECKPOINT.md`, `docs/RECOVERY-RUNBOOK.md`. | **Current production recovery point since 2026-09-20 (D1b.2): `ascend-backup-20260920T104952Z-post-009.ascbk`** (ledger 001–009), re-proven by full R1b (61/61) and two-leg R1c on 17.6 (31/31). The R1 artifact `ascend-backup-20260919T120457Z-r1b.ascbk` is now HISTORICAL (pre-009), retained; it was proven F1–F18 on PGlite 18.3 (R1b) and on a real PostgreSQL 17.6 server over both restore paths (R1c), consumed by the application as the restored `ascend_app` login. **Same-version HTTP application boot is NOT COVERED**, intentionally and permanently: the production database transport requires TLS verification against the production trust chain, and `core/db/tls.ts` is not modified for recovery testing. | Outside R1, tracked separately and NOT prerequisites for R1: R2 (vault/file recovery), B2 (off-machine replication), deployment-secret escrow, August-backup disposition (retention cleanup). |
...
- `core/db/backup.ts` dynamically captures columns of five listed tables, so newer user credential columns may be included; **invitations and prospect_notes tables are not**. Do not mistakenly claim all credential data is omitted. Recovery tests need current credential/invitation/note fixtures. *(Superseded 2026-09-19: `core/db/backup.ts` is retired; see the R1a row.)*
...
| Dependency R1 Current recovery proof — **COMPLETE (2026-09-19)**: R1a complete, R1b complete, R1c complete; same-version HTTP boot explicitly NOT COVERED | PostgreSQL only. Split by owner decision (2026-09-19): **R1a** canonical, encrypted, isolated recovery machinery with fixtures in every table (no production contact); **R1b** take and prove the first current recovery point (one authorized read-only production read); **R1c** same-version (17→17) restore and server-level boot — REQUIRED before PostgreSQL disaster recovery is called production-proven | Deployment-mode facts, safe fixtures | F1–F18 (`docs/DEPENDENCY-R1-CONTRACT.md` §5); notes/invites/credentials/events sequence/rows survive; encrypted at rest; current runbook (`docs/RECOVERY-RUNBOOK.md`). PGlite 18 restoring production 17 is NOT a same-version proof. |
| Infrastructure I1 — serve production outside iCloud and outside the working tree | Move Ascend OS's serving and build state out of the iCloud-synced development tree, so a build cannot come from uncommitted files, cannot collect iCloud duplicates or evictions, and always carries a commit ↔ BUILD_ID pairing | — (independent; recorded 2026-09-21 during the D1 deployment, which accepted the current location for that deployment only) | Measured: production runs from `~/Desktop/…/apps/os` inside iCloud Drive; the previous live `.next` held 3,696 iCloud duplicates; the error log shows past in-place-build crash loops and an iCloud-class `scandir -11`; the previous build contained uncommitted Slice 1C with no commit identity. A built `.next` embeds its absolute path, so the move must rebuild in the new location. Not mixed with any feature slice. |
| Dependency R2 Vault/file recovery | Recovery and versioning of the external vault and every other file-backed business state the application depends on | — (independent of R1; owner decision 2026-09-19: not folded into R1, 4C or 6B) | Discovered in the R1 pre-flight: the vault (`ASCEND_VAULT_PATH`) holds clients/CRM, documents, uploads, SOPs, invoices, time log, audits, automations, portal submissions/invites and approvals; it lives in iCloud Drive, is NOT a git repository, is outside every database backup, and has NO proven recovery path. iCloud is sync, not backup. Not implemented in R1. |
| Dependency D1 — prospect source-correct mutations: **COMPLETE (2026-09-20, owner-accepted) and DEPLOYED (2026-09-21)**. D1a complete `4276f89` · D1b.1 complete `eb0f8b6` · D1b.2 complete `326a407` | Promotion and removal honor the selected owner store, with truthful partial outcomes and a converging retry contract. Postgres-mode removal is ARCHIVAL (`009_prospect_archival.sql`), never raw DELETE; sales holds exactly two archival columns. Promotion and archival are serialized by a prospect-scoped transaction advisory lock — proven on PostgreSQL 17.6: without it 2 concurrent promotions made 2 clients and 6 made 6; with it, one client and one `prospect.promoted`. **Schema 009 is LIVE in production** (ledger head `009_prospect_archival.sql`, verified 51/51; 0 prospects archived, 0 business rows changed). **Current recovery point: `ascend-backup-20260920T104952Z-post-009.ascbk`** (full R1b 61/61, two-leg R1c 31/31 on 17.6). Pre-009 artifact `…20260919T120457Z-r1b.ascbk` is HISTORICAL, retained. | R1; explicit source configuration | Vault/Postgres cases, retained notes, no silent closed-won disagreement; no destructive migration. **Application code: DEPLOYED 2026-09-21** — candidate `ba648d3` (runtime-identical to `43eec93`), build `2NCS2fCKGRoKqk3tUd4F8`, PID 18362, 8 s outage, post-deploy smoke 30/30, zero business-data change (`docs/DEPLOYMENT-D1-CHECKPOINT.md`). **Carried verification debt:** (1) sales archival not witnessed against a real production sales account — none exists; (2) **RT-1** (`docs/RECOVERY-RUNBOOK.md` §7): three R1b/R1c assertions assume every prospect is active and compare the active-set `listProspects` to the TOTAL — they must compare the active count before, or with, the first real archival; (3) ~~D1a/D1b.1 application code undeployed~~ — resolved 2026-09-21, deployed. |
...
*(Answered 2026-09-20 by D1a/D1b.2: production runs `ASCEND_PROSPECT_SOURCE=postgres` and its ledger was inspected read-only — 001–008, then 001–009 after D1b.2.)* Before D1, inspect deployment configuration/ledger through an authorized read-only path. Only if that does not settle the operational policy is an owner decision required: must vault-only prospect operation remain supported, or is Postgres the sole supported production prospect mode? Code supports both on reads and inconsistently on writes; documentation alone cannot choose the supported business operating mode.
