...
| Dependency D1 Prospect source-correct mutations — **D1a COMPLETE (`4276f89`), D1b.1 COMPLETE (this checkpoint), D1b.2 NOT STARTED** | Promotion/deletion honor selected owner; truthful partial outcomes and retry contract. D1b replaced Postgres-mode deletion with ARCHIVAL (`009_prospect_archival.sql`) and closed D1a's concurrency debt with a prospect-scoped transaction advisory lock, proven on a real PostgreSQL 17.6 server: without the lock, 2 concurrent promotions produced 2 clients and 6 produced 6; with it, one client and one `prospect.promoted`. | R1; explicit source configuration | Vault/Postgres cases, retained notes, no silent closed-won disagreement; no destructive migration. **⛔ DEPLOYMENT INVARIANT: D1b.1 IS NOT DEPLOYABLE until D1b.2 applies migration 009 to production.** Production is at ledger head 008 and lacks `archived_at`/`archived_by`; the canonical reader now filters on `archived_at IS NULL`, so deploying first breaks EVERY prospect read. Order is 009 → production first, application afterwards. D1b.2 also owns the new encrypted recovery artifact and the full R1b + two-leg R1c re-proof — the pre-009 artifact must NOT be carried forward as the current recovery point (`F2`, `F4.digest.prospects`, `F5`, `F13`, `F17` all move; `F4.digest.prospects` changes even with zero archived rows). |
