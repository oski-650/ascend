...
export async function compileOperatorBrief(): Promise<string> {
  ...
  for (const s of states) {
    if (s.activePhaseIndex === null) continue; // skip launched
    ...
    const active = s.phases[s.activePhaseIndex].label;
    ...
    const dtlStr =
      dtl === null
        ...
        : dtl < 0
          ...
          : `${dtl}d to launch`;
    projectLines.push(
      `- **${s.clientName}** — ${s.overallProgress}% · active: ${active} · ${dtlStr} · ${fmtHours(totalSeconds)} tracked · EHR ${ehr !== null ? fmtUsd(ehr) + "/hr" : "—"} · health ${health.score} (${health.tier.replace("_", " ")})`
    );
  }
