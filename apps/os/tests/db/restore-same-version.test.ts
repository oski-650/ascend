...
import { contractTables, type RecoveryContract } from "@/core/recovery/contract";
import {
  assertEphemeralServer, assertIsolatedEnvironment, assertRestorableTarget, compareManifests,
  createRestoreRoles, ephemeralSocketConfig, manifestOf, openEphemeralSocketTarget, restoreInto,
  verifyBehaviour, type EphemeralServer, type Manifest, type RestoreTarget,
} from "@/core/recovery/restore";
import { ledgerOf, openRecoveryArtifact } from "@/tests/support/recovery-artifact";
...
type Artifact = { dump: string; customDump: Buffer; globals: string; source: Manifest; contract: RecoveryContract; tables: string[] };
...
function leg(name: "leg1" | "leg2", restore: (cluster: R1cCluster, target: RestoreTarget) => Promise<void>) {
  describe(`${name === "leg1" ? "LEG 1 · portable SQL" : "LEG 2 · pg_restore of the custom dump"} on PostgreSQL ${EXPECTED_VERSION}`, () => {
    ...
    beforeAll(async () => {
      ...
      restored = await manifestOf(adminRestore.target, artifact.contract.manifestSql);
    }, 600_000);
    ...
    it("F6/F12/F15 · behaviour holds for every restored organization", async () => {
      ...
      let checks = 0;
      for (const o of orgs) {
        const results = await verifyBehaviour(adminRestore.target, o.id, artifact.contract);
    ...
    describe("the application consumes it — read-only, as the restored ascend_app login, on a guarded clone", () => {
      ...
      beforeAll(async () => {
        ...
        // 2 · write-refusal triggers on every table the contract's manifest counts.
        ...
        manifestBefore = await manifestOf(smokeAdmin.target, artifact.contract.manifestSql);
        ...
        appPool = new Pool({ ...ephemeralSocketConfig(server, { user: "ascend_app", password: appPassword, database: "r1c_smoke" }), max: 1 });
      }, 600_000);
      ...
      /** One application connection, proven to be the isolated server's read-only `ascend_app` session. */
      const appSession = async <T,>(fn: (s: ProfileSession) => Promise<T>): Promise<T> => {
        ...
        try {
          ...
          return await fn({ query: async <R,>(sql: string, params?: unknown[]) => { appQueries++; return { rows: (await c.query(sql, params)).rows as R[] }; } });
        } finally {
          c.release();
        }
      };
      ...
      it("AC3 · ascend_app authenticates over the socket and every connection proves the isolated server", async () => {
        const n = await appSession(async (s) => Number((await s.query<{ n: number }>("SELECT 1 AS n")).rows[0].n));
        expect(n).toBe(1);
      });

      // RT-3B · the application half of the proof is the CONTRACT's application verification profile,
      // run as the application roles over the restored ascend_app login — never today's readers, whose
      // SQL may assume a newer schema than this artifact's ledger. Same resolution path on both legs.
      it("AC4 · the contract's application verification profile passes, as ascend_app, on this leg", async () => {
        const admin: ProfileSession = { query: async <R,>(sql: string, params?: unknown[]) => ({ rows: (await smokeAdmin.client.query(sql, params)).rows as R[] }) };
        profile = await appSession((app) => runApplicationProfile(artifact.contract, {
          admin, app, ownerEmail: OWNER_EMAIL!, ownerPassword: OWNER_PASSWORD!,
          sourceEventOrderDigest: artifact.source.get("F6.events.order.digest"),
        }));
        note(`${name}.application_profile`, { profile: profile.profile, checks: profile.checks.length,
          failed: profile.checks.filter((c) => !c.ok).map((c) => c.id), measured: profile.measured, appQueries });
        expect(profile.profile).toBe(artifact.contract.applicationProfile);
        expect(profile.checks.filter((c) => !c.ok).map((c) => `${c.id} (${c.detail})`)).toEqual([]);
        expect(appQueries).toBeGreaterThan(0);
      }, 300_000);

      it("reads reached the server and were flushed to statistics, and no tuple was written (positive control)", async () => {
        await appPool.end();
      ...
      it("AC5 · a deliberate write is refused on every path", async () => {
        ...
        for (const t of artifact.tables) expect(outcomes[`superuser.${t}.insert`], t).toBe("P0001:guard");
      }, 120_000);
      ...
      it("AC5 · nothing changed: no tuple was written and the clone's manifest is identical", async () => {
        ...
        const c = compareManifests(manifestBefore, await manifestOf(smokeAdmin.target, artifact.contract.manifestSql));
...
describeIfR1c("R1c · the production artifact on a real PostgreSQL 17.6 server", () => {
  ...
  beforeAll(async () => {
    ...
    // RT-3: the same opening and contract resolution as R1b (tests/support/recovery-artifact.ts).
    const opened = openRecoveryArtifact(ARTIFACT!, KEYRING!, LEGACY_CONTRACT);
    ...
    artifact = {
      dump: opened.member("ascend-public-portable.sql").toString("utf8"),
      customDump: opened.member("ascend-public.dump"),
      globals: opened.member("globals-nopw.sql").toString("utf8"),
      source: opened.source,
      contract: opened.contract,
      tables: contractTables(opened.contract),
    };
    note("artifact.key_id", header.keyId);
    note("artifact.format", header.format);
    note("contract", { kind: opened.contract.kind, id: opened.contract.id, manifestSha256: opened.contract.manifestSha256,
      ledgerHead: opened.contract.ledgerHead, tables: artifact.tables.length });
