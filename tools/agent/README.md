...
Run `ASCEND_AGENT=codex npm run agent -- prove TASK_ID --as codex` in the claimed builder worktree. It checks the clean exact tree, runs or reuses valid receipts for the task's selected gates, and stops at the owner checkpoint only if selected recovery suites need it. The owner then runs the one command printed by the checkpoint in the same worktree; the owner email is entered at a visible, non-echoing prompt. The command records no Coordinator state, never unblocks, and does not freeze. See [PROOF-ORCHESTRATION.md](PROOF-ORCHESTRATION.md) for the state machine and authority boundaries.
