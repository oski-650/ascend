...
import { slugOf, validateTask, validateResult, LOCKED } from './lib/schema.mjs';
...
import { runGates, GateError } from './lib/gates.mjs';
...
async function gateNames(s,cwd){const sha=await baseline(s,cwd);try{return new Set(Object.keys(JSON.parse(await git(['show',`${sha}:tools/agent/gates.json`],{cwd}))));}catch{fail('GATE_REGISTRY_INVALID','baseline gate registry missing or invalid',3);}}
...
function checkResultRules(t,r){const errors=validateResult(r);if(errors.length)fail('RESULT_INVALID',errors.join(', '),2);
  if(r.verdict==='FIX_REQUIRED'&&!r.findings.some(f=>f.blocking))fail('FINDING_REQUIRED','FIX_REQUIRED needs a blocking finding');
  if(r.verdict==='ACCEPT'){
    ...
    for(const f of t.open_findings)if(!r.prior_findings.some(x=>x.id===f.id&&x.disposition==='resolved'))fail('PRIOR_FINDING',f.id);
  }
}
...
async function invalidate(cwd,id,agent,session){await casMutate(cwd,'invalidate',agent,s=>{const t=get(s,id);return transition(s,'invalidate',{id,as:agent,reason:'REVIEW_OBJECT_MOVED',note:'review ref, tree or manifest mismatch',actor:agentCtx(agent,session).actor});});fail('REVIEW_INVALIDATED',id,6);}
...
async function freeze(cwd,id,agent,sess){
  ...
  const result=await casMutate(cwd,'freeze',agent,async s=>{
    const t=get(s,id);if(t.state==='PUBLISHED'&&t.review?.sha===await revParse('HEAD',cwd))return {task:id,round:t.round,sha:t.review.sha,branch:t.review.branch,noop:true};
    if(!['IMPLEMENTING','FIX_REQUIRED'].includes(t.state)||t.builder!==agent||t.claim?.worktree_id!==wt)return {ok:false,code:'NOT_CLAIMED',reason:id};
    const d=await baseline(s,cwd),head=await revParse('HEAD',cwd);let b=d;
    if(!await isAncestor(d,head,cwd)){b=t.baseline_sha;if(!await isAncestor(b,d,cwd)||!await isAncestor(b,head,cwd))return {ok:false,code:'BASELINE_REWRITTEN',reason:id};const g=await diffNames(b,d,cwd);if(g.some(p=>[...t.write_paths,...t.read_paths].some(x=>matches(x,p))))return {ok:false,code:'BASELINE_MOVED_CONFLICT',reason:g.join(', ')};}
    const dirtyPaths=await statusPaths(cwd),changes=[...new Set([...(await diffNames(b,head,cwd)),...dirtyPaths])];
    const invalid=changes.filter(p=>!t.write_paths.some(x=>matches(x,p))||t.blocked_paths.some(x=>matches(x,p))||s.config.forbidden_name_patterns.some(x=>matches(x,p)));
    if(invalid.length)return {ok:false,code:'UNAUTHORIZED_PATHS',reason:invalid.join(', ')};
    const round=t.round+1;if(dirtyPaths.length&&!dry){await git(['add','-A','--',...dirtyPaths],{cwd});await git(['commit','-m',`${id} r${round}\n\nAscend-Task: ${id}\nAscend-Round: ${round}\nAscend-Baseline: ${b}`],{cwd});}
    if((await statusPaths(cwd)).length&&!dry)return {ok:false,code:'TREE_DIRTY_AFTER_COMMIT',reason:id};
    const sha=await revParse('HEAD',cwd),tree=await treeOf(sha,cwd),previous=t.rounds.at(-1);
    if(previous&&(previous.sha===sha||previous.tree===tree))return {ok:false,code:'NO_CHANGES',reason:id};
    const full=await buildChanges(b,sha,cwd),unauthorized=full.filter(x=>!t.write_paths.some(p=>matches(p,x.path))||t.blocked_paths.some(p=>matches(p,x.path))||s.config.forbidden_name_patterns.some(p=>matches(p,x.path))||x.new_mode==='160000');
    if(!full.length)return {ok:false,code:'NO_CHANGES',reason:id};
    if(unauthorized.length)return {ok:false,code:'UNAUTHORIZED_PATHS',reason:unauthorized.map(x=>x.path).join(', ')};
    const evidence=await runGates(t.required_gates,{sha,cwd});
    if(dry)return {task:id,dry_run:true,sha,changes:full.length};
    const branch=`review/${t.slug}-r${round}`,ref=`refs/heads/${branch}`,existing=await lsRemote(ref,cwd);
    if(existing&&existing!==sha)return {ok:false,code:'BRANCH_COLLISION',reason:branch};
    const manifest={task:id,round,branch,sha,tree,baseline:b,previous_round_sha:previous?.sha||null,builder:agent,session:sess,published_seq:s.events.length+1,changes:full,changes_sha256:changesSha(full),delta_from_previous:await deltaFromPrevious(previous?.sha,sha,cwd),gates:evidence,gate_evidence_kind:'builder_attested'};
...
async function reviewStart(cwd,id,agent,sess){
  ...
  if(!await verifyReview(s,t,cwd))await invalidate(cwd,id,agent,sess);
  ...
  const path=arg('--worktree')||resolve(cwd,'..',`ascend-review-${t.slug}-r${t.round}`);
...
async function reviewResult(cwd,id,agent,sess,input,{emitOnly=false}={}){
  ...
  if(t.state!=='REVIEWING'||r.round!==t.round||r.reviewed_sha!==t.review?.sha||r.reviewed_tree!==t.review?.tree||s.files[`reviews/${id}/r${r.round}.result.json`]){
    await casMutate(cwd,'stale-result',agent,s2=>{const x=get(s2,id);event(s2,'review.stale_rejected',{actor:agentCtx(agent,sess).actor,from:x.state},x,{round:r.round,sha:r.reviewed_sha});return {task:id};});fail('REVIEW_STALE',id,6);
  }
  if(!await verifyReview(s,t,cwd))await invalidate(cwd,id,agent,sess);
  ...
  const result=await casMutate(cwd,'review-result',agent,async s2=>{
    ...
    if(!await verifyReview(s2,task,cwd)){transition(s2,'invalidate',{id,as:agent,reason:'REVIEW_OBJECT_MOVED',note:'review moved during result',actor:agentCtx(agent,sess).actor});return {task:id,invalidated:true};}
    const filled={...r,task:id,reviewer:agent,session:sess,result_seq:s2.events.length+1};s2.files[`reviews/${id}/r${r.round}.result.json`]=filled;
    const x=transition(s2,r.verdict==='ACCEPT'?'accept':'fix',{id,as:agent,result:filled,actor:agentCtx(agent,sess).actor});if(!x.ok)return x;return {task:id,verdict:r.verdict};
...
async function promoteCommand(cwd,id,agent,humanActor=null){
  ...
  const reviewer=t.reviewer,reviewSession=t.review?.reviewer_session;
  if(!await verifyReview(s,t,cwd))await invalidate(cwd,id,reviewer,reviewSession);
  ...
  for(let n=0;n<3;n++){
    ...
    if(plan.mode==='overlay'){
      ...
      try{await runGates(t.promotion_gates,{sha:plan.sha,cwd});}finally{await git(['switch',branch||'--detach',...(branch?[]:[before])],{cwd});}
    }
...
async function main(){
  ...
  if(cmd==='claim'){
    const result=await casMutate(cwd,'claim',agent,async s=>{const d=await baseline(s,cwd),task=get(s,id);for(const p of task.prerequisites){const prior=s.tasks[p];if(prior?.state!=='PROMOTED'||!await isAncestor(prior.promoted.sha,d,cwd))return {ok:false,code:'PREREQUISITE',reason:p};}const claim={agent,session:sess,host:hostname(),worktree_id:sha256(hostname()+cwd),seq:s.events.length+1};const x=transition(s,'claim',{...ctx,claim,baseline_sha:d,resume:has('--resume')});if(!x.ok)return x;return {task:id};});if(!result.ok)fail(result.code,result.reason);emit({claimed:id,session:sess});return;
  }
  ...
  if(cmd==='ingest'){const file=id,r=JSON.parse(await readFile(file,'utf8'));if(!r.task||!r.reviewer||!r.session)fail('INGEST_FORMAT','task, reviewer and session required',2);await reviewResult(cwd,r.task,r.reviewer,r.session,file);return;}
  if(cmd==='promote'){await promoteCommand(cwd,id,agent);return;}
  if(['withdraw','reopen','block'].includes(cmd)){
    const op=cmd,reason=arg('--reason');if(cmd!=='withdraw'&&!reason)fail('REASON_REQUIRED','--reason required',2);
