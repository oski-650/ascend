...
import { revParse, statusPaths, treeOf, gitPath } from './git.mjs';
const exec=promisify(execFile);
export class GateError extends Error{constructor(message,exit=7){super(message);this.exit=exit;}}
export async function runGates(names,{sha,cwd,registryPath=join(cwd,'tools/agent/gates.json')}={}){
  const registry=JSON.parse(await readFile(registryPath,'utf8'));const evidence=[];
  for(const name of names){const gate=registry[name];if(!gate||!Array.isArray(gate.cmd)||!gate.cmd.length||!Number.isInteger(gate.timeout_s))throw new GateError(`unknown gate ${name}`,1);
    ...
    const item={name,command:gate.cmd,cwd:gate.cwd,exit_code,duration_ms:Date.now()-started,log_sha256:sha256(output),log_tail:Buffer.from(output).subarray(-4096).toString(),ran_on:{sha,tree:await treeOf(sha,cwd),clean_before:true,clean_after:true}};
