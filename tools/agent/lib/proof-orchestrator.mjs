...
export function safeProofLines(output) {
  ...
  if (!lines[0]?.startsWith('candidate: ') ||
    !lines.at(-1)?.startsWith('aggregate: ') && !lines.at(-1)?.startsWith('OWNER_CHECKPOINT: '))
    throw Error('proof progress is incomplete');
  return lines;
}

export async function proveTask({ cwd, task, id, owner, actor }) {
  if (id !== 'COORD-PROOF-001' || task.id !== id) throw Error('proof task mismatch');
  if (!['IMPLEMENTING', 'FIX_REQUIRED'].includes(task.state) || task.builder !== 'codex' ||
    task.claim?.agent !== 'codex' || task.claim.worktree_id !== sha256(hostname() + cwd))
    throw Error('proof task is not claimed in this worktree');
  if (owner ? actor !== 'owner' : actor !== 'codex') throw Error('proof actor invalid');
  if ((await statusPaths(cwd)).length) throw Error('proof candidate must be clean');
  const head = await revParse('HEAD', cwd);
  let output;
  try {
    const result = await exec(process.execPath,
      [join(cwd, 'apps/os/scripts/proof-orchestrator.mjs'), ...(owner ? ['--owner'] : [])],
